...
const actions = {
  ...
  async getProducts({ commit }: UserUIDataActionContext): Promise<IProduct[]> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Ошибка на сервере, нет товаров.';
  ...
  async doOrder(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Ошибка на сервере при создании заказа.';
  ...
  toggleFavorite(
    { commit }: UserUIDataActionContext,
    productId: string
  ): void {
