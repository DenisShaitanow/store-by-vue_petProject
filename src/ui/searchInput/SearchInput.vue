  ...
  <div :class="wrapperClasses">
    <div :class="$style.search" @focus="handleFocus">
      ...
      <input
        ...
        :aria-activedescendant="activeIdx >= 0 ? `${idBase}-option-${filteredOptions[activeIdx].id}` : undefined"
        role="combobox"
      />
      <button
        v-if="value"
    ...
    >
      ...
      <li v-if="showEmpty" :class="$style.dropdownEmpty">
        Не найдено
      </li>
    </ul>
  </div>
...
<script setup lang="ts">
    import { ref, computed, onMounted, onUnmounted, watch, withDefaults, defineProps, defineEmits } from 'vue'
    ...
    !!props.value && filteredOptions.value.length === 0
    )
