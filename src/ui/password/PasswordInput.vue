<template>
  <div :class="[$style.container, { [$style.container_large]: largeSize }]">
    ...
    >
      ...
      <RevealElement @click="toggleInputVisibility" :visible="showPassword" />
    </div>
    ...
    <p v-else-if="error && isLoginPage" :class="[$style.text, $style.text_tip, $style.text_error]">
      {{ errorText }}
    </p>
  </div>
</template>
...
<script setup>
  ...
  const placeholderText = computed(() =>
    isRegisterPage.value ? 'Придумайте надежный пароль' : 'Введите ваш пароль'
  );
