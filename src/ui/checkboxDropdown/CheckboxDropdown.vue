<template>
  ...
  <div
    v-if="staticMode"
    :class="[$style.staticContainer, className, staticMode && $style.staticMode]"
  >
    ...
    <div :class="$style.content">
      ...
      <div :class="$style.optionsList">
        ...
        <!-- Tree mode -->
        <div v-else-if="treeMode" :class="$style.treeContainer">
          <ExpandableList
                ...
                >
                  ...
                  <div :class="$style.checkboxContainer">
                    ...
                    <label
                      ...
                      :class="[
                        $style.checkboxIcon,
                        getCheckboxState(option) === 'indeterminate' &&
                          $style.checkboxIcon_indeterminate,
                      ]"
                    />
                  </div>
                    ...
                    >
                      ...
                      <div :class="$style.checkboxContainer">
                        ...
                        <label
                          ...
                          :class="[
                            $style.checkboxIcon,
                            getCheckboxState(child) === 'indeterminate' &&
                              $style.checkboxIcon_indeterminate,
                          ]"
                        />
                      </div>
        ...
        >
          <template #default>
            <div v-for="option in filteredOptions" :key="option.value" :class="$style.option">
              <CheckboxGroupUI
                :field-names="[option.label]"
                :selected-items="
                  selectedValuesComputed.includes(option.value) ? [option.label] : []
                "
  ...
  >
    <template #trigger="{ toggle }">
      ...
      >
        ...
        <svg
          :class="[$style.chevron, isOpen && $style.chevron_open]"
          width="20"
          height="20"
          viewBox="0 0 20 20"
          fill="none"
...
<script setup lang="ts">
  ...
  // Props with defaults
  const props = withDefaults(defineProps<CheckboxDropdownProps>(), {
  ...
  // Local state
  const internalSelectedValues = ref<(string | number)[]>(defaultSelectedValues?.value ?? []);
  ...
  const getCheckboxState = (option: CheckboxOption) => {
    if (!treeMode.value || !option.children) {
      return selectedValuesComputed.value.includes(option.value) ? 'checked' : 'unchecked';
    }
  ...
  const handleTreeOptionToggle = (option: CheckboxOption) => {
    const newSelected =
      treeMode.value && option.children
        ? (() => {
            const childValues = getAllChildren(option);
            const allSelected = childValues.every((v) => selectedValuesComputed.value.includes(v));
            return allSelected
